#!/usr/bin/env python3
"""Build a SQLite database for ACS 2024 B19013 data for selected ZCTAs."""
...
OUTPUT_DB = RAW_DIR / "acs_2024_b19013_selected_zctas.db"
TABLE_NAME = "b19013_2024_selected_zctas"
GROUP_PREFIX = "B19013"
...
def build_db(input_csv: Path, output_db: Path, table_name: str, zips: list[str]) -> None:
    ...
    group_cols = [c for c in header_df.columns if c.startswith(GROUP_PREFIX)]
    if not group_cols:
        raise ValueError(f"No {GROUP_PREFIX} columns found in input CSV.")
    usecols.extend(group_cols)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build SQLite DB for ACS 2024 B19013 for selected ZCTAs."
    )
